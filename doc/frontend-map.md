...
- 所有角色都可用置於頂部導覽列正中央的全域 `HomeUiSurfaceSwitcher.vue` 切換；切換同時套用查詢、收退料、產能、主檔、帳目、品質與圖片維護：
  - Workspace UI：`WorkspaceSystemSurface.vue` 以 Modern 頂部欄承載混合流程；快速作業 route async 載入 `WorkbenchUiSurface.vue` 並關閉其內嵌版頭，收／退料總檢視雖由 transaction-only `WorkbenchManagementSurface.vue` 承載三欄結果與篩選，但路由、區域語意及教學皆歸屬現場工作台；桌面版總檢視會把左欄縮成只保留四個模式切換按鈕，手機版仍維持整列響應式切換；資料與產能 route 使用完整維護頁面，互動產生的 query 會持續保存 `ui_surface=workspace`
  ...
  - Workspace 只有 `/inventory/overview` 使用 `WorkbenchManagementSurface.vue`，並以 `transactions-only` 模式呈現收退料總檢視；桌面左側導覽縮成只保留四個模式切換按鈕，中間是完整帳目表格，右側是篩選工具，手機則把切換列與工具區改為縱向響應式配置。`/production/*` 與 `/master/*` 一律交給現行 canonical 維護頁，不再套用舊 Workbench 管理殼層；guest 只能讀取允許的總檢視，user 仍不能直接進入 Admin 帳目／品質 route
  ...
  - `playwright.config.ts` 與 `tests/visual/workbench.visual.spec.ts` 使用確定性 API mock，在真實手機 390×844、Tablet 1024×900、Notebook 1366×900、Desktop 1920×1080 四個 project 比對現行 Workspace 收退料、Datecode／序號查詢、收退料總檢視、canonical 治具維護及帳目案件頁。測試同時檢查 URL-backed 狀態、總檢視桌面縮窄切換列、手機響應式切換列、現行 list/detail selector 與整頁不得水平溢位；baseline 以 `workspace-*` 命名，舊 `workbench-*` 快照已退役。`npm run test:visual` 驗證 baseline，`npm run test:visual:update` 只在人工檢查並批准新版面時更新
