# Fixture-M Lite Project Tasks

## 2026-08-25 大型前端頁面第二輪拆分

- [x] onboarding picker、guided tour、release notice、production batch modal，以及 Master 品質快速修正／永久刪除都改接 `UiModalShell`；Master 兩個 dialog 另拆成獨立元件。
- [x] `useConfigurationReportState.ts` 集中 configuration report 的 route query、draft／applied filters、分頁與正式匯出狀態；`InventoryRelationsPage.vue` 降至約 975 行。
- [x] `useMasterCrudActions.ts` 與 `useMasterEntityDeletion.ts` 接手 CRUD、啟停、永久刪除及刪除最後一筆後退頁；`MasterPage.vue` 降至約 1,472 行。
- [x] `InventoryRelationsPage.vue`、`MasterPage.vue`、`ProductionPage.vue` 樣式移至 `styles/surfaces`，保留原 global／scoped 語意。
- [x] `BatchImportPanel.vue` 的解析、預覽、送出狀態分至三個 composable，元件降至約 892 行。
- [x] 完整前端測試 62 個檔案／166 tests 通過，production build 通過。

## 2026-08-24 專案進度與文件同步

- [x] Modern／Form UI 已從首頁模式擴展為全系統 surface；`App.vue` 統一切換 `/search`、收退料、產能、主資料、帳目、品質與圖片維護，兩套介面共用原有穩定 routes。
- [x] Form UI 的標題與說明會依目前工作區動態顯示；工作區索引改用完整文字並分成「日常作業／設定維護／系統管理」，route 切換後由 router `scrollBehavior` 回到模組／篩選頂端。
- [x] admin／user 可在 compact surface switcher 選擇個人的登入預設，偏好依帳號存於 `localStorage`；目前 surface 存於 `sessionStorage`，guest 仍固定以 Form UI 為預設。
- [x] Form UI 高資料量清單改用後端 50／100 筆分頁：治具、機種、站點、客戶、使用者、圖片狀態、機種站點 mapping 與治具需求都只保留目前頁面資料。
- [x] Form 產能編輯不再預載三份完整 master options；`FormRemoteAutocomplete.vue` 以 debounce 按需查詢並限制最多 20 筆候選，具備鍵盤選取與過期回應保護。
- [x] Form 使用者維護顯示授權客戶摘要，新增／編輯時由 `FormUserCustomerScopePicker.vue` 搜尋與複選客戶；Form 儲存前至少要求一個 `allowed_customer_id`。
- [x] Form 產能新增兩種 spreadsheet paste：機種站點兩欄、治具需求四欄；後端 preview 先分類新增／相同／衝突／錯誤，需求數量差異只有經應用程式確認後才以 `overwrite_existing=true` 更新，未貼上的關聯不刪除。
- [x] Modern 查詢在未輸入條件時改載入 customer-scoped 治具總清單，顯示庫存、狀態、儲位與啟用狀態；選取後沿用既有 lazy fixture detail，並支援「載入更多」。
- [x] `FormReportOperations.vue` 由多領域實作元件縮為 route dispatcher；收退料、產能、主資料分別拆到 `FormTransactionOperations.vue`、`FormProductionOperations.vue`、`FormMasterDataOperations.vue`，共用驗證、全頁匯出與錯誤文字集中於 `utils/formOperations.ts`。
- [x] `InventoryBatchEntryGrid.vue` 的新增列與 Enter 導航改為只在目前 grid root 內尋找欄位，避免多個 grid 實例互相搶焦點。
- [x] 同步 `ARCHITECTURE.md`、`ARCHITECTURE_LANDING.md`、`frontend-map.md`、`backend-map.md`、`map.md`、`to-update.md` 與 `update.md`；歷史日期與舊驗證快照保留不改寫。
- [x] 2026-08-24 完整驗證：backend `119 passed`；frontend `53` 個測試檔／`152 passed`；production build 通過。

## 2026-08-04 P2 前端頁面拆分與 Pydantic 序列化相容

- [x] `InventoryRelationsPage.vue` 由約 4,061 行降至 3,127 行；篩選、結果區與圖片 dialog 分別移至 `InventoryReportFilters.vue`、`InventoryReportResults.vue`、`FixtureImageDialog.vue`，頁面保留 route query、draft／applied filters、分頁與匯出協調。
- [x] 保留既有報表 characterization tests，並補上 customer-scoped 圖片載入、object URL 清理、一般聯動篩選、手機／桌面分頁與元件邊界測試；另以 390px、1280px 實際頁面確認收合、卡片／表格切換及無整頁水平溢位。
- [x] `MasterPage.vue` 由約 3,410 行降至 2,925 行；toolbar 抽至 `MasterToolbar.vue`，ledger 與 quality 載入、選取、儲存及錯誤狀態分別由 `useMasterLedger.ts`、`useMasterQuality.ts` 負責，頁面保留 route/tab、customer scope 與跨區導覽協調。
- [x] `backend/app/schemas/common.py` 改用 Pydantic 2 `field_serializer`，移除 deprecated `ConfigDict(json_encoders=...)`，並保留 timestamp JSON 僅輸出 `YYYY-MM-DD` 的 API 契約。
- [x] 新增共用 schema、巢狀庫存回應、FastAPI `response_model` 與 schema import deprecation guard 測試；當時完整驗證為 backend `113 passed`（不再出現 `json_encoders` 警告）、frontend `35` 個測試檔／`102 passed`，production build 通過。

## 2026-08-04 Token 過期返回登入流程

- [x] `api/core.ts` 將 JSON、文字與 Blob transport 的 authenticated `401` 統一導入 session-expired flow；登入 API 自己的 `401` 不會被誤判。
- [x] 第一個 `401` 立即清除記憶體及 `sessionStorage` 的 session／目前客戶，並行失敗只觸發一次「登入已逾時」提示與一次導頁。
- [x] 導頁前保存目前內部 route 的 path、query 與 hash；重新登入或進入訪客模式成功後返回原頁，再清除暫存 return path。
- [x] 手動登出會清除可能殘留的 expiry return path，不會在下次登入誤返舊頁。
- [x] 新增 transport 並行 `401`、非登入狀態 `401`、return path 保存／消耗與 `/login` 排除測試。
- [x] 前端完整測試：`31` 個測試檔、`97 passed`；production build 通過。

## 2026-08-04 全域 Modal 鍵盤與焦點行為

- [x] 新增共用 `UiModalShell.vue`，統一提供 `role="dialog"`／`alertdialog`、`aria-modal`、accessible label、初始焦點、Tab／Shift+Tab 焦點循環、Esc 與背景點擊關閉。
- [x] Modal 開啟時將 body 內其他內容設為 `inert`，關閉時恢復原狀並把焦點送回觸發按鈕。
- [x] Modal stack 只讓最上層處理鍵盤事件；收退料草稿確認可疊在全域 Modal 上，不會一次關閉兩層或提早解除背景 inert。
- [x] 全域收退料與匯出中心改用共用 shell，關閉按鈕為初始焦點；既有 `SystemConfirmDialog` 也改接相同 shell，維持 `alertdialog` 語意與原視覺。
- [x] 新增 `UiModalShell.test.ts` 與 `AppGlobalModals.test.ts`，並保留 `SystemConfirmDialog.test.ts` 回歸測試。
- [x] 當時前端完整測試：`29` 個測試檔、`93 passed`；production build 通過；最新基線見上方 token 過期項目。

## 2026-08-04 1280px 頂欄快捷操作

- [x] `AppTopbar.vue` 在 `1024px–1366px` compact desktop 區間保留「選單、目前客戶、收／退料」三個高頻入口。
- [x] 匯出中心、新手教學、統計、客戶切換及登出等低頻操作繼續收進抽屜選單；低於 `1024px` 時只保留手機既有的選單與目前客戶資訊。
- [x] compact desktop 的收退料文案縮短為「收／退料」，完整桌面仍顯示「治具收/退料」；訪客／唯讀 session 不會出現操作快捷鍵。
- [x] 新增 `AppTopbar.test.ts`，覆蓋 compact shell 的三個保留項目與唯讀權限行為。
- [x] 當時前端完整測試：`27` 個測試檔、`88 passed`；production build 通過；最新基線見上方全域 Modal 項目。

## 2026-08-04 收退料快速新增一列

- [x] `BatchImportPanel.vue` 新增常駐「快速新增一列」，提供治具 autocomplete、datecode／編號、數量及「加入待送清單」。
- [x] 快速新增只接受目前客戶下的有效治具，治具代碼比對不分大小寫；無效輸入會停留在表單並顯示提示。
- [x] 純數字 identifier 繼續沿用共用正規化規則；加入後轉為既有批次草稿列，使用相同預覽、退料庫存預檢、重複合併及送出流程。
- [x] 最終送出仍只使用既有 receipts／returns batch payload，沒有恢復另一套舊式單筆交易 API。
- [x] 補上快速列加入、identifier 正規化、批次 API payload 與無效治具拒絕測試；frontend 完整測試 `86 passed`，production build 通過。
- [x] 修正 customer-scope 圖片相容流程的跨客戶治具代碼唯一性查詢，避免 MySQL `ONLY_FULL_GROUP_BY` 使治具 autocomplete 清單端點回傳 500；`backend/tests/test_services.py` 共 `43 passed`，並以目前 MySQL 8.4 容器驗證分組查詢。

## 2026-08-04 手機報表分頁與結果定位

- [x] 手機報表預設每頁由 50 筆降為 20 筆，手機提供 20／50，桌面維持 50／100。
- [x] `page_size` 仍同步至 URL；跨越手機／桌面 breakpoint 時會換成該版面允許的最接近筆數並重新載入。
- [x] 手機結果摘要與已套用條件改為 topbar 下方的 sticky toolbar，長摘要與條件 chips 採單列橫向滑動，避免遮住過多卡片內容。
- [x] 保留手機初次進入及套用條件後自動收合篩選的既有行為。
- [x] 補上手機 20／50 分頁與 sticky 結構測試；frontend 完整測試 `84 passed`，production build 通過。

## 2026-08-04 報表主檢視切換（已撤回）

- [x] 依後續需求移除「全部治具／全部機種／全部站點」三個主檢視按鈕與非主體「不選」語意。
- [x] 治具、機種、站點恢復為一般聯動篩選器，空值分別表示全部治具、全部機種、全部站點。
- [x] 移除 URL `basis`、API `report_dimension` 與 repository 單一主資料投影；頁面、摘要、分頁及 CSV／XLSX 均恢復關聯明細列，同一治具可因多個配置重複出現。
- [x] 保留 `populated_columns` 自動收合無資料欄位、欄位偏好及匯出可見欄位功能。
- [x] 回退後完整驗證：backend `113 passed`、frontend `35` 個測試檔／`102 passed`，production build 通過。

## 2026-08-04 手機主資料操作收斂

- [x] 390px 主資料頁的 KPI 改為單列 compact chips，可水平滑動且不顯示捲軸，清單可在首屏直接出現。
- [x] 手機版七個主資料分頁改為單一「主資料功能」下拉選單，並以「資料維護／系統管理」分組。
- [x] 圖片選取／上傳、CSV／JSON、返回搜尋與新手導覽等低頻功能集中至「更多操作」。
- [x] 手機進入 summary／edit／create 明細後隱藏 KPI、分頁與批次工具，只保留返回清單、目前資料類型／項目及適用的編輯入口。
- [x] 以 390 × 844 視窗驗證清單首屏、功能切換、「更多操作」與新增明細流程；frontend tests 與 production build 通過。

## 2026-08-04 全系統日期顯示收斂

- [x] 盤點日期輸入、API 回應、交易寫入、查詢篩選及 CSV／XLSX 匯出，確認業務日期皆為日粒度。
- [x] 頂欄最近收／退料移除時分顯示，改用共用 `YYYY-MM-DD` formatter。
- [x] 資料維護與產能頁的「建立時間／更新時間」統一改為「建立日期／更新日期」。
- [x] 技術性 timestamp 僅保留於內部稽核排序與兩分鐘重複送出防護，不在 UI 顯示時分秒。
- [x] 驗證：frontend `25` 個測試檔、`81 passed`，production build 通過；backend 日期匯入／匯出 targeted tests `3 passed`。

## 2026-08-04 治具圖片 customer scope

- [x] 圖片正式儲存路徑改為 `FIXTURE_IMAGE_DIR/<customer_id>/<fixture_code>.<ext>`，避免跨客戶同碼覆寫。
- [x] 圖片 GET 改為必填 `customer_id`，並套用 admin／user assigned-customer scope；guest 維持全客戶唯讀。
- [x] 舊版平面圖片只在 fixture code 全系統唯一時相容讀取；跨客戶同碼不共享 legacy 圖片。
- [x] 治具更名補上檔案搬移 rollback；永久刪除治具後清理該 customer 圖片。
- [x] 前端所有圖片 blob 請求帶入目前 customer，並在 customer 切換時重新載入。
- [x] 補上跨客戶同碼、scope 拒絕、legacy 歧義、更名 rollback 與永久刪除清理測試。
- [x] 當時完整驗證：backend `108 passed`、frontend `80 passed`、frontend production build 通過；最新基準見本文件最上方。

## 2026-08-04 文件同步稽核

- [x] 對照目前 source、路由、資料模型、migration、前端元件與 `doc/` 文件。
- [x] 修正 architecture landing 的 admin customer scope、雙欄儲位、`/login` route 與 migration head。
- [x] 修正 Docker 文件中不存在的預設密碼及 host/container port 說明。
- [x] 將 `to-update.md` 的 5 月原始需求轉為完成／待辦狀態，不再保留失效的 Typora 本機圖片路徑。
- [x] 補上當時的 2026-08 更新摘要與驗證基準：backend `105 passed`、frontend `79 passed`、frontend build 通過。

## 2026-08-03 報表治具狀態與開站明細教學

- [x] 報表新增 `已啟用／已停用／所有治具` 狀態篩選，預設為已啟用；條件同步至後端分頁、聯動選項、URL 與 CSV／XLSX 匯出。
- [x] 已啟用模式仍保留沒有治具本體的未配置機種／站點列，避免預設條件掩蓋配置缺口；已停用模式只列停用治具。
- [x] 最大開站量結果維持站點摘要，點擊「查看治具明細」後以報表表格展開治具名稱、需求、庫存、可開站、庫存狀態與瓶頸標示。
- [x] user/admin 新手教學新增獨立 7 步 `庫存配置報表`；完整詳細版同步加入報表步驟，guest 報表教學文案更新至最新功能。

## 2026-08-03 撤回報表頁收退料輸入 Mockup

- [x] 從 `InventoryRelationsPage.vue` 移除 `InventorySheetInputMockup` 的 import 與畫面入口。
- [x] 刪除只供介面預覽、未串接正式資料的 `InventorySheetInputMockup.vue`。
- [x] 報表收退料篩選、明細展示、匯出，以及既有正式收料／退料作業維持不變。

## 2026-07-31 主資料與全域互動整理

- [x] 主資料頁初次載入不再自動選取第一筆；清單選取後先顯示唯讀摘要，只有按下「編輯」才建立可修改表單狀態。
- [x] 主資料新增／編輯／取消／重載流程明確區分 `summary / edit / create`，儲存完成後回到唯讀摘要。
- [x] 抽出 `MasterReadonlySummary.vue`，降低 `MasterPage.vue` 與 `MasterDetailPanel.vue` 的呈現責任。
- [x] 將手機報表卡片與收退料明細呈現抽至 `InventoryReportMobileCards.vue`，繼續縮減 `InventoryRelationsPage.vue`。
- [x] 新增共用 `confirmState.ts + SystemConfirmDialog.vue`，替換 App、查詢、主資料、產能及批次收退料的原生 `window.confirm`。
- [x] 系統確認視窗支援 `alertdialog`、Esc、背景取消、焦點進入與 Tab 焦點循環；危險操作有一致的紅色確認語意。
- [x] 登出後使用 `router.replace` 導回 `/login`；登入／訪客入口完成後導向 `/search`。
- [x] feature routes 改為動態 import，並以輕量 `LoginRoutePage.vue` 避免登入路徑預先載入首頁報表 bundle。
- [x] Toast 關閉按鈕補上 `aria-label="關閉通知"`。
- [x] 補上確認狀態、系統確認視窗及主資料唯讀摘要測試，並通過前端 build。

## 2026-07-31 報表篩選後自動精簡欄位

- [x] configuration-report page response 新增 `populated_columns`，以全部符合篩選的資料判定哪些欄位實際有值，不受目前分頁限制。
- [x] 報表套用條件後，將使用者勾選欄位與 `populated_columns` 取交集，整欄無資料時自動隱藏；下一次篩選有資料時自動恢復。
- [x] 欄位選擇器保留原始勾選偏好，自動隱藏欄位標示「無資料」，並列出本次自動隱藏數量與名稱。
- [x] 桌面表格、手機摘要卡與篩選結果匯出使用同一份有效欄位，避免畫面與匯出不一致。
- [x] 補上完整篩選集合欄位統計後端測試及前端有效欄位單元測試。

## 2026-07-31 庫存配置報表後端聚合

- [x] 新增 `GET /inventory/configuration-report`，由後端統一完成關鍵字／治具／機種／站點／水位／儲位／配置狀態／收退料方向／來源／日期篩選、排序與分頁。
- [x] 報表 read model 合併已配置需求、未綁定治具、未配置機種站點、未映射機種及未映射站點，並回傳低水位、未配置、總庫存、客供與自購統計。
- [x] 新增 `GET /inventory/configuration-report/options`，依使用者實際 `priority` 順序回傳聯動治具／機種／站點／水位選項。
- [x] 新增 `GET /inventory/configuration-report/export`，由後端輸出完整篩選結果 CSV／XLSX、目前可見欄位及可選收退料明細，不受目前頁碼限制。
- [x] `InventoryRelationsPage.vue` 移除六組完整資料與 200 筆逐頁 transaction overview 載入，改成當頁 50 筆 server paging。
- [x] 報表新增客供／自購來源篩選、客供／自購庫存欄位與摘要；明細及匯出包含交易來源。
- [x] 欄位選擇器新增 `現場庫存`、`配置檢查`、`完整報表` 三組預設，套用後仍可逐欄自訂並保存瀏覽器偏好。
- [x] 報表與匯出新增 `可開站` 欄位；後端先以完整機種站點需求集合計算瓶頸產能，再套用治具等報表篩選。
- [x] 移除報表篩選區重複的客戶選擇器，統一由共用頂欄切換客戶。
- [x] 報表明確區分 draft／applied 條件；有差異時顯示尚未套用數量，按鈕改為 `套用條件`，產能只使用已套用的機種／站點。
- [x] 任一草稿條件或日期變更時清除舊產能結果，避免無效日期或尚未套用條件與舊結果並存。
- [x] 手機初次進入即收合完整篩選，只保留關鍵字、更多條件入口及結果摘要。
- [x] 依最新需求取消低水位、未配置、今日收料、今日退料一鍵篩選卡，桌面與手機入口皆移除；相關條件仍可在一般篩選區選擇後套用。
- [x] 手機報表改用治具摘要卡片呈現庫存、配置、機種、站點與儲位，不再依賴桌面寬表水平捲動。
- [x] 報表分頁支援每頁 50／100 筆、上一頁／下一頁與直接跳頁，`page_size` 同步至 URL。
- [x] 產能設定的 `加入站點` 與 `加入治具需求` 表單預設收合；新增、編輯或教學進入對應步驟時才展開。
- [x] 新增 `0015_configuration_report_indexes.py`，建立交易客戶／方向／日期及治具／來源的報表複合索引。
- [x] 補上後端聚合、來源、交易日期、聯動選項與 XLSX 匯出測試。

## 2026-07-30 首頁查詢／報表雙模式

- [x] `/search` 由 `SearchHomePage.vue` 提供 `查詢`（原治具／機種查詢）與 `報表`（治具庫存與配置報表）兩個可即時切換的首頁模式。
- [x] 查詢工作台會把查詢類型、輸入值、已執行查詢及選中結果保存在首頁層；切到報表時精確套用剛才選中的治具／機種，切回查詢時還原輸入值與選取，不再因元件卸載而清空。
- [x] 若輸入框內容尚未重新搜尋、與上次結果不同，切到報表時改用目前可見文字作關鍵字，不會錯用舊的治具／機種選取。
- [x] guest 預設進入報表；admin / user 首次預設進入查詢。
- [x] admin / user 可將目前模式設為自己的登入後預設，偏好依登入帳號保存於瀏覽器；guest 下次進入仍固定預設報表。
- [x] `home_mode=query|report` 保存目前首頁模式；查詢頁有未儲存修改時，切到報表前會先確認。
- [x] 訪客首頁移除 `治具 / 站點 / 機種` 檢視方式及卡片展開流程。
- [x] 訪客首頁改為密集型報表：上方固定篩選區，下方固定表頭、斑馬紋資料表。
- [x] 客戶由共用頂欄單一入口選擇；報表條件包含關鍵字、治具、機種、站點、水位狀態、產線／部門儲位，以及收退料方向、客供／自購來源與日期。
- [x] 水位篩選支援 `低水位與缺料 / 只看低水位 / 只看缺料 / 只看正常`。
- [x] 篩選改為依使用者選擇順序聯動：第一個非空條件是主條件，後續欄位只列出相容選項，後選條件不反向縮限第一條件。
- [x] 聯動條件以 `1 / 2 / 3` 順位標籤完整顯示，標籤可直接移除；分享 URL 以 `priority` 保存實際選擇順序。
- [x] 報表欄位包含總庫存、客供庫存、自購庫存、最低水位、儲位、機種、站點、需求數量及配置狀態。
- [x] 報表預設顯示全部欄位，以淡藍選項卡與圓形勾選標記支援逐欄顯示／隱藏及一鍵全部顯示；至少保留一欄，並以瀏覽器本機偏好記住選擇。
- [x] 新增 `匯出篩選結果`：由後端輸出所有符合目前篩選條件的 CSV／XLSX，不受分頁限制，但只包含目前可見欄位；完成後持續顯示筆數、欄位數與檔名。
- [x] 報表篩選區新增 `計算最大開站數`；選好機種即啟用，站點為「全部站點」時列出該機種所有已綁定站點，指定站點時只顯示單站，並呈現各站最大開站數與瓶頸治具。
- [x] 最大開站數結果預設只顯示站點與數量；瓶頸治具改為各站獨立展開，避免全部站點結果過度擁擠。
- [x] 報表新增今日收料、今日退料、指定日期收料及指定日期退料篩選；指定日期只有搭配收／退料模式才生效，日期不完整或順序錯誤時顯示欄位錯誤並停用查詢。
- [x] 套用收／退料篩選後，結果工具列提供「展示收／退料明細」勾選項；展開時依治具顯示類型、來源、日期、單號、datecode／編號與數量，同一治具在當頁只展開一次。
- [x] 勾選收／退料明細時，`匯出篩選結果` 會在所有符合篩選的報表資料後附加交易明細欄位；同一治具的交易不因多個機種／站點配置而重複匯出。
- [x] 報表結果區顯示實際已套用條件與結果筆數，避免 draft 輸入與上一輪結果混淆。
- [x] 報表治具代碼改為圖片入口，點擊後以對話框載入既有治具圖片，並處理載入中、無圖片、焦點限制、關閉後焦點還原與 URL 回收。
- [x] 未選客戶時改為明確提示選擇客戶，不再誤顯示成尚未配置。
- [x] `更多功能` 不再重複顯示治具／機種查詢；`/search/detail` 保留給舊連結、新手教學及跨頁返回相容。
- [x] guest 不再顯示或進入產能管理／治具需求設定；直接開啟產能路由會返回報表並顯示原因。
- [x] guest 點擊新手教學會先看到版本選擇，可選 7 步 `查詢工作台＋報表` 快速版或 18 步完整唯讀版；完成或中途結束後還原啟動前 URL 與篩選狀態。
- [x] guest 教學選單清楚標示收退料／資料維護／產能設定需 User/Admin，以及帳號／客戶／帳目／資料品質僅限 Admin。
- [x] 篩選與分頁狀態同步至 URL，表格在窄螢幕可水平捲動；手機初次進入及套用後自動收合篩選，保留關鍵字、快速篩選、更多條件與結果摘要。

## 2026-07-30 篩選式庫存關聯頁初版（已由訪客報表式首頁取代）

- [x] 將原 mockup 整理為正式 `InventoryRelationsPage.vue`，並設為 `/search` 首頁。
- [x] 依目前客戶串接既有治具、機種、站點、機種站點、治具需求、總庫存與 identifier 庫存 API。
- [x] 支援 `治具 / 站點 / 機種` 主體切換、複合篩選、未配置篩選、分頁、URL 狀態同步及客戶切換重載。
- [x] 機種完整性與機種展開明細一律使用未受篩選縮限的完整關聯資料，避免站點篩選造成假的 `未配置`。
- [x] 站點主體的機種對應治具維持使用篩選後資料，只呈現目前條件下符合的組合。
- [x] 保留既有藍白視覺與 CSS 語意；藍色負責互動與選取，綠 / 橘 / 紅保留給庫存及配置狀態。
- [x] 正式頁面可導向治具 / 站點資料維護、機種產能設定，並可開啟全域收退料 modal；訪客維持唯讀。
- [x] 原 `SearchWorkspacePage.vue` 移至 `/search/detail`，並收進頂欄與手機選單的 `更多功能 > 治具 / 機種詳細查詢`。
- [x] 舊詳細查詢頁導向總檢視或產能頁時，`return_to` 會保留 `/search/detail` 的完整查詢狀態。
- [x] 舊 `/inventory/filter-view` 與 `/inventory/relations` 路徑統一 redirect 到 `/search`。
- [x] 新手教學中屬於舊詳細查詢頁的步驟改指向 `/search/detail`。
- [x] `MasterPage` 支援 `fixture_id / model_id / station_id` route query，自正式頁面進入維護時可直接選中對應資料。
- [x] 同步更新 `ARCHITECTURE.md`、`task.md`、`frontend-map.md`、`backend-map.md`、`map.md` 與 `color-mapping.md`。

## 2026-07-29 Datecode / 編號庫存依來源分組

- [x] 治具查詢頁的 datecode/編號庫存改為先顯示 `客供` / `自購` 來源區塊，再於區塊內列出編號與該來源數量。
- [x] 不再於每一個 datecode/編號標籤後重複顯示來源。
- [x] 來源數量為 0 時不建立該來源區塊；若舊資料同一編號同時存在兩個來源，會分別列在兩個來源區塊。
- [x] 新增來源分組顯示工具測試。

## 2026-07-29 收退料來源篩選與詳細版新手教學

- [x] `GET /inventory/transactions/overview` 新增 `ownership_type=customer_supplied|self_purchased` 篩選，後端分頁總數與明細使用同一條件。
- [x] 收 / 退料總檢視的進階篩選新增 `來源：全部 / 客供 / 自購`，並將條件同步到 route query，重新整理與翻頁後仍保留。
- [x] 精簡版收退料教學補上客供 / 自購來源選擇，以及總檢視進階來源篩選說明。
- [x] 新增第六張 `全系統按鈕與操作說明` 詳細版教學卡，逐區說明首頁、查詢、批次收退料、總檢視、主資料與產能管理的主要按鈕。
- [x] 匯出中心的收退料明細自定義條件新增 `來源：全部 / 客供 / 自購`，正式匯出與匯出預覽 API 使用相同的 item-level 篩選。
- [x] 精簡版教學補上匯出來源說明；詳細版新增匯出資料集、格式、範圍、每個篩選欄位、欄位預覽與操作按鈕說明。
- [x] 教學走到匯出進階篩選時，自動切到 `收退料明細 + 自定義條件`，確保來源欄位實際可見。
- [x] 教學選單把完整詳細版移到最上方推薦區，並在導覽卡持續顯示詳細版名稱與 `1 / 33` 步驟狀態，避免和精簡版混淆。
- [x] 新增 onboarding flow 結構與必要步驟測試。

## 2026-07-29 客供 / 自購庫存拆分

- [x] 確認正式資料庫目前庫存公式：`總庫存 = 客供庫存 + 自購庫存`；3,713 種治具均無來源負數或公式不一致。
- [x] `GET /inventory/stock`、`GET /inventory/alerts`、`GET /inventory/identifier-stock-summary` 新增 `customer_supplied_qty` 與 `self_purchased_qty`。
- [x] 依業務規則確認客供 / 自購收料分開且 datecode 不跨來源重複；退料維持 `fixture + identifier` 總量檢核。
- [x] 收退料頁 KPI、現有庫存、低水位與治具查詢詳情可同時檢視總庫存、客供庫存、自購庫存。
- [x] datecode/編號庫存標籤只顯示有庫存的來源；單一來源不再顯示另一來源的 `0`，雙來源時才同時列出。
- [x] 批次退料預覽維持 datecode 總庫存的交易前 / 後數量，來源只用於交易分類。
- [x] 後端新增來源庫存拆分與總庫存公式測試。
...
### 2) 儲位策略收斂
- [x] warehouse 相關表先收斂為 fixture 文字儲位；後續已由 `0012`／`0013` 改成 `line_storage_location` 與 `department_storage_location` 雙欄並移除舊單欄
...
- 新手教學入口位於全域 shell，admin / user / guest 點擊後都會先開啟 flow picker。
- admin / user 維持原分類教學；guest 可選 `查詢工作台與庫存配置報表` 7 步快速版或涵蓋所有可查看功能的 18 步完整唯讀版。
- guest picker 另列出登入後才可使用的 User/Admin 與 Admin-only 功能，避免權限範圍不明。
- `/search` 由 `SearchHomePage` 提供查詢／報表切換：guest 預設報表，admin / user 初始預設查詢並可保存個人登入預設。
- `SearchWorkspacePage` 已移除 `相近編號` 提示框，詳細查詢頁保留最近收 / 退料治具快捷入口與結果區導向流程。
- `MasterPage` 已有前端分頁；`SearchWorkspacePage` 也已改為 bounded search contract + `load more`。
- `/search/detail` 保留為舊連結、新手教學與跨頁返回使用的直接查詢路由；日常入口統一使用 `/search` 的雙模式切換。
...
- `npm run build` 目前可通過。
- `.venv\\Scripts\\python.exe -m pytest -q` 目前為 `105 passed`。
...
- 治具需求送出前會以目前庫存預估儲存後最大開站數；production routes 現在只開放 admin / user，guest 可由報表依機種計算全部站點或指定單站的唯讀開站結果。
...
- 既有目標 MySQL 最後一次已記錄的實機驗證為 Alembic `0014_fixture_deletion`，transaction item 的 `fixture_id` 可為空並使用 `ON DELETE SET NULL`。
- repo source 的目前 Alembic head 為 `0015_configuration_report_indexes`；部署前仍須在目標環境套用並驗證此 revision。
...
- [x] 頂部欄新增 `更多功能` 下拉，包含 `治具 / 機種詳細查詢 / 收退料總檢視 / 資料維護 / 產能管理`
- [x] Logo 可點擊回 `/search`
