...
def list_fixtures(customer_id: int | None = Query(default=None), db: Session = Depends(get_db)):
    return MasterService(db).list_fixtures(customer_id=customer_id)


@router.get("/fixtures/export")
def export_fixtures(customer_id: int = Query(...), db: Session = Depends(get_db)):
...
@router.post("/fixtures/import", response_model=ImportResultRead, dependencies=[Depends(require_permission("manage"))])
def import_fixtures(
...
@router.put("/fixtures/{fixture_id}", response_model=FixtureRead, dependencies=[Depends(require_permission("manage"))])
def update_fixture(
    fixture_id: int,
...
def list_models(customer_id: int | None = Query(default=None), db: Session = Depends(get_db)):
    return MasterService(db).list_models(customer_id=customer_id)


@router.get("/models/export")
def export_models(customer_id: int | None = Query(default=None), db: Session = Depends(get_db)):
...
@router.post("/models/import", response_model=ImportResultRead, dependencies=[Depends(require_permission("manage"))])
def import_models(
...
@router.put("/models/{model_id}", response_model=MachineModelRead, dependencies=[Depends(require_permission("manage"))])
def update_model(
    model_id: int,
...
def list_stations(customer_id: int | None = Query(default=None), db: Session = Depends(get_db)):
    return MasterService(db).list_stations(customer_id=customer_id)


@router.get("/stations/export")
def export_stations(customer_id: int | None = Query(default=None), db: Session = Depends(get_db)):
...
@router.post("/stations/import", response_model=ImportResultRead, dependencies=[Depends(require_permission("manage"))])
def import_stations(
    customer_id: int | None = Query(default=None),
    payload: CsvImportPayload = ...,
    session: SessionContext = Depends(require_permission("manage")),
...
@router.put("/stations/{station_id}", response_model=StationRead, dependencies=[Depends(require_permission("manage"))])
def update_station(
    station_id: int,
...
def create_owner(
    ...
    service = MasterService(db)
    try:
        return service.create_owner(payload, actor=session)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
...
@router.put("/owners/{owner_id}", response_model=OwnerRead, dependencies=[Depends(require_permission("manage"))])
def update_owner(
    owner_id: int,
    payload: OwnerUpdate,
    session: SessionContext = Depends(require_permission("manage")),
    ...
    service = MasterService(db)
    try:
        return service.update_owner(owner_id, payload, actor=session)
    except ValueError as exc:
        message = str(exc)
        status_code = status.HTTP_404_NOT_FOUND if message.endswith("not found") else status.HTTP_400_BAD_REQUEST
        raise HTTPException(status_code=status_code, detail=message) from exc
