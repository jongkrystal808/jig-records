...
- [x] 新增「收 / 退料總檢視」頁面
- [x] 總檢視支援：全部查詢 / 序號查詢
- [x] 總檢視支援篩選：收退料類型、日期區間、治具編號、單號、datecode、序號、操作人員
...
### 10) 收退料資料模型調整
- [x] 治具主檔只分為 `datecode` 與 `serial` 兩大管理類型
- [x] 治具主檔不再區分 `客供 / 自購`
- [x] `客供 / 自購` 改為收退料交易明細欄位 `ownership_type`
- [x] `datecode` / `serial` 共用同一組收退料交易表
- [x] 收退料記錄需能區分 `datecode` 類與 `serial` 類
- [x] 收退料流程需檢核並展示 `ownership_type`

#### 10.1) 模型定案

- `fixtures.manage_type`
  - `datecode`
  - `serial`
...
- 選擇治具後，依 `fixtures.manage_type` 決定輸入表單
- `datecode` 類：顯示 `datecode + quantity`
- `serial` 類：顯示 `serial_number`，必要時支援多筆序號
