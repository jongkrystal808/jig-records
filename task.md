...
- [x] 篩選條件包含客戶、關鍵字、治具、機種、站點、水位狀態及產線／部門儲位。
- [x] 水位篩選支援 `低水位與缺料 / 只看低水位 / 只看缺料 / 只看正常`。
- [x] 篩選改為依使用者選擇順序聯動：第一個非空條件是主條件，後續欄位只列出相容選項，後選條件不反向縮限第一條件。
- [x] 第一優先條件會顯示在篩選標題列；分享 URL 以 `priority` 保存實際選擇順序。
- [x] 報表欄位包含庫存、最低水位、儲位、機種、站點、需求數量及配置狀態。
- [x] 報表預設顯示全部欄位，以淡藍選項卡與圓形勾選標記支援逐欄顯示／隱藏及一鍵全部顯示；至少保留一欄，並以瀏覽器本機偏好記住選擇。
- [x] 新增 `匯出篩選結果`：下載所有符合目前篩選條件的資料，不受分頁限制，但只包含目前可見欄位；欄位順序、中文狀態與頁面一致，輸出 Excel 相容 CSV。
- [x] 報表篩選區新增 `計算最大開站數`；選好機種即啟用，站點為「全部站點」時列出該機種所有已綁定站點，指定站點時只顯示單站，並呈現各站最大開站數與瓶頸治具。
- [x] 最大開站數結果預設只顯示站點與數量；瓶頸治具改為各站獨立展開，避免全部站點結果過度擁擠。
- [x] 報表新增今日收料、今日退料、指定日期收料及指定日期退料篩選；指定日期只有搭配收／退料模式才生效，未選模式時日期不參與其他條件。
- [x] 報表治具代碼改為圖片入口，點擊後以對話框載入既有治具圖片，並處理載入中、無圖片、關閉與 URL 回收。
- [x] 未選客戶時改為明確提示選擇客戶，不再誤顯示成尚未配置。
- [x] `更多功能` 不再重複顯示治具／機種查詢；`/search/detail` 保留給舊連結、新手教學及跨頁返回相容。
- [x] guest 不再顯示或進入產能管理／治具需求設定；點擊新手教學會直接開始合併的 `查詢工作台＋報表` 教學，不再開啟分類選單。
- [x] 篩選與分頁狀態同步至 URL，表格在窄螢幕可水平捲動。
...
- 新手教學入口位於全域 shell：admin / user 點擊後開啟 flow picker；guest 點擊後直接開始合併的查詢工作台與報表教學。
- admin / user 維持原分類教學；guest 只使用 `查詢工作台與庫存配置報表` 單一 7 步流程。
